...
// 사용자가 해당 사이트에서 어떤 행동을 했는지 서버 저장
...
function setCookie (name, value, hours){
  ...
  // /shopping 으로 작성 할경우 해당 경로에서만 쿠키 유효 
...
function getCookie(name){
  ...
  for(let i = 0; i < cookies.length; i++) {
    ...
    // 쿠키 문자열에서 특정이름과 일치하는 쿠키 값이 있는지 확인
    // indexOf : 문자열 내에서 특정 문자열을 찾는 method
    // nameEqu 변수가 쿠키 문자열의 시작(0) 위치애서 시작되는지를 확인 => True
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  closeBtn.addEventListener('click',function(){
    popup.style.display = 'none';
      ...
      // setCookie라는 함수에 name, value, hours 라는 
      // 매개변수 값 세 개를 넘겨준다.
